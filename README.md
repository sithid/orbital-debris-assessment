...
# Windows but using Git Bash or WSL etc
...
you have any issues with the CLI, you can run each notebook in order (00-06) to achieve the same results. The CLI was designed to make it easier to update the entire pipeline, and make sure everything builds correctly, but the CLI is not actually required. You can run any notebook independently as long as you have the required datasets and outputs from previous steps. When running manually for the first time, you may have to set each notebook kernal to the environment you created.
